(function($){
    ...
    function allproducts(dataqty,currentpage,obj){
        ...
        $.post(global.apiBaseUrl+'products', par, function(_data){
            ...
            $tbody.append($.map(res, function(item){
                return `<tr><td>${item.barcode}</td><td>${item.name}</td><td>${item.type}</td><td>${item.specification}</td><td>${item.price}</td><td>${item.vipprice}</td><td>${item.stockprice}</td><td>${item.repertory}</td><td>${item.putaway}</td><td>${item.supplier}</td><td><button>修改</button> <button>删除</button></td></tr>`
            }));
            //遍历tbody实现隔行换色分页
    ...
    $('tbody').on('click',function(e){
        ...
        currentpage = $paging.children('.gaol').html()-1;
        //修改商品数据
        ...
        if(target.innerHTML == '删除'){
            var barcode = currentTr.children[0].innerHTML;
            $.post(global.apiBaseUrl+'delproduct', {barcode:barcode}, function(_data){
    ...
    $addbtn.click(function(){
        ...
        BCbtn.onclick = function(){
            redact.style.display = '';
            zhez.style.display = '';
            var _newdata = writeIn();
            if(_newdata.putaway>0){
                _newdata = Object.assign({},_newdata,{status:'上架'})
            }else{
               _newdata = Object.assign({},_newdata,{status:''}) 
            }
            $.post(global.apiBaseUrl+'insertproduct', _newdata, function(_data){
                allproducts(dataqty,currentpage);
                page(currentpage);
            })
        }
    ...
    function writeIn(){
        ...
        newdata.supplier = input[9].value;
        return newdata;
    }
    //隔行变色
    function interleave(){
